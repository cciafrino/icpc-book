...
struct bruteforce { // values in nodes
	...
	void root(int cur, int p = -1) {
		pars[cur] = p;
		for (auto i: tree[cur]) {
			if (i.st != p) root(i.st, cur);
		}
	}
	bruteforce(vector<vector<pair<int,int>>> _tree): tree(_tree), vals(sz(tree),unit), pars(sz(tree)) {
		root(0);
	}
	bool dfsModify(int cur, int target, int val, int p=-1) {
		...
		for (auto i: tree[cur]) {
			if (i.st == p) continue;
			alongPath |= dfsModify(i.st, target, val, cur);
		}
	...
	int dfsQuery(int cur, int target, int p = -1) {
		...
		for (auto i: tree[cur]) {
			if (i.st == p) continue;
			res = f(res, dfsQuery(i.st, target, cur));
		}
		if (res != unit) {
...
void testAgainstBrute(int n, int iters, int queries) {
    for (int trees = 0; trees < iters; trees++) {
        ...
        for (auto i : graph) {
            ...
            tree1[i.first].push_back({i.second,0});
            tree1[i.second].push_back({i.first,0});
        }
		int root = rand() %n;
        HLD<int,false> hld(tree1);
        ...
        for (int itr = 0; itr < queries; itr++) {
            ...
            if (rng == 0) {
				...
				hld.query_path(a, b);
				hld2.queryPath(a, b);
				cout<<"QUERY PATH "<<a<<" "<<b<<endl;
				cout<<hld.query_path(a,b)<<" "<<hld2.queryPath(a,b)<<endl;
                assert(hld.query_path(a, b) == hld2.queryPath(a, b)); 
            } else if (rng == 2) {
				int a = rand() % n;
				cout<<"QUERY SUB "<<a<<endl;
				cout<<hld.query_subtree(a)<<" "<<hld2.querySubtree(a)<<endl;
				assert(hld.query_subtree(a) == hld2.querySubtree(a)); 
			}
        }
    }
