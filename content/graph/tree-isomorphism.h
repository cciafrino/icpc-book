...
struct tree_t { ///start-hash
	...
	int dfs(int tsz, int v, int p) { ///start-hash
		for (int u : adj[v]) if (u != p) {
			if (2*sz[u] <= tsz) continue;
			return dfs(tsz, u, v);
		} return cen[0] = v;
	} ///end-hash
	void find_cenroid(int v) { ///start-hash
		int tsz = dfs_sz(v, -1);
		cen[1] = dfs(tsz, v, -1);
		for (int u : adj[cen[0]]) if (2*sz[u] == tsz)
			cen[1] = u;
	} ///end-hash
	int hash_it(int v, int p = -1) { ///start-hash
	...
	ll get_hash(int v = 0) { ///start-hash
		find_cenroid(v);
