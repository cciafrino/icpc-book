...
P eval(const P& a, const vector<num>& x) {
	int N = int(x.size());
	if (!N) return {};
...
P interp(const vector<num>& x, const vector<num>& y) {
...
P taylor_shift(P a, int N, int K) {
	P P(N), Q = a;
	P[0] = 1;
