 ...
 * Date: 
 * License: 
 * Source: https://github.com/victorsenam/caderno/blob/master/code/zeta_dor.cpp
 * Description: Use when you want to calculate c[i] = sum (a[j] * b[k]) such that (j | k) == i and (j & k) = 0.
 * Usage: (if the size of a and b is (1 << k))
 * tf(k, a, a_);
 * tf(k, b, b_);
 * conv(k, a_, b_, ans);
 * itf(k, ans, c);
