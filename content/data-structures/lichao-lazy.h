...
struct lichao_lazy{
	...
	T query( T x , node *cur , T l, T r){///start-hash
	...
	void add( line li, node *&cur, T l, T r){ ///start-hash
		if(cur == nullptr){ 
			cur = new node(li);
			return;
		}
	...
	void propagateLine(node *&cur, T l, T r){///start-hash
		if(cur == nullptr) return;
		T mid = ( l + r ) >> 1;
	...
	void addSegment( line li, node *&cur, T l, T r, T lseg, T rseg){///start-hash
	...
	void updateSegment( line li, node *&cur, T l, T r, T lseg, T rseg){ ///start-hash
		if(r < lseg || l > rseg) return;
		if(cur == nullptr) cur =  new node;
		if(lseg <= l && r <= rseg){
			cur->apply(li);
