...
vector<ull> convolve(const vector<ull>& as, const vector<ull>& bs) {
	static constexpr unsigned M0 = M0::M, M1 = M1::M, M2 = M2::M, M3 = M3::M, M4 = M4::M;
	static const modnum<M1> INV_M0_M1 = modnum<M1>(M0).inv();
	...
	for (int i = 0; i < len_as; ++i) as2[i] = as[i];
	for (int i = 0; i < len_bs; ++i) bs2[i] = bs[i];
	const vector<modnum<M2>> cs2 = FFT2.convolve(as2, bs2);
	vector<modnum<M3>> as3(len_as), bs3(len_bs);
	for (int i = 0; i < len_as; ++i) as3[i] = as[i];
	for (int i = 0; i < len_bs; ++i) bs3[i] = bs[i];
	const vector<modnum<M3>> cs3 = FFT3.convolve(as3, bs3);
	vector<modnum<M4>> as4(len_as), bs4(len_bs);
	for (int i = 0; i < len_as; ++i) as4[i] = as[i];
	for (int i = 0; i < len_bs; ++i) bs4[i] = bs[i];
	const vector<modnum<M4>> cs4 = FFT4.convolve(as4, bs4);
	vector<ull> cs(len_as + len_bs - 1);
	for (int i = 0; i < len_as + len_bs - 1; ++i) {
		cs[i] = garner<ull>(cs0[i], cs1[i], cs2[i], cs3[i], cs4[i]);
	}
	return cs;
}
